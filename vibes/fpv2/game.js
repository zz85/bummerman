...
let musicPlaying = false, musicGain = null;
...
function init() {
  ...
  document.addEventListener('keydown', e => { 
    ...
    if (e.code === 'KeyV' && locked && !splitScreen) {
  ...
  document.addEventListener('pointerlockchange', () => {
    ...
    if (musicGain) musicGain.gain.value = locked ? 0.12 : 0;
  });
...
function explode(bomb) {
  ...
  if (bomb.owner && bomb.owner.bombs !== undefined) {
    bomb.owner.bombs++;
  ...
  for (const enemy of enemies) {
    ...
    if (enemy.moveTimer <= 0) {
      ...
      if (inDanger) {
        ...
        // Hunter AI: pathfind toward nearest player
        let bestDir = enemy.dir;
        let bestDist = Infinity;
        const targets = splitScreen ? players.filter(p => !p.dead) : [player];
        ...
        for (let d = 0; d < 4; d++) {
          ...
          if (!collides(nx, nz) && !isInDanger(nx, nz)) {
            for (const t of targets) {
              const dist = Math.abs(t.x - nx) + Math.abs(t.z - nz);
              if (dist < bestDist) {
                bestDist = dist;
                bestDir = d;
              }
    ...
    const targets = splitScreen ? players.filter(p => !p.dead) : [player];
    const distToPlayer = Math.min(...targets.map(t => Math.abs(ex - t.x) + Math.abs(ez - t.z)));
    ...
    if (splitScreen) {
      for (const p of players) {
        if (!p.dead && Math.abs(ex - p.x) < CELL * 0.5 && Math.abs(ez - p.z) < CELL * 0.5) {
          p.dead = true;
          playerMeshes[players.indexOf(p)].visible = false;
          playSound('death');
...
function updatePowerups(dt) {
  for (const p of [...powerups]) {
    ...
    if (splitScreen) {
      // Check all players in split-screen
      for (const pl of players) {
        if (pl.dead) continue;
        if (Math.abs(p.mesh.position.x - pl.x) < CELL * 0.5 && Math.abs(p.mesh.position.z - pl.z) < CELL * 0.5) {
          playSound('powerup');
          if (p.type === 'bomb') pl.bombs++;
          else if (p.type === 'blast') pl.blast++;
          else if (p.type === 'speed') pl.speed += 0.8;
          scene.remove(p.mesh);
          powerups = powerups.filter(x => x !== p);
          break;
        }
      }
    } else if (Math.abs(p.mesh.position.x - player.x) < CELL * 0.5 && Math.abs(p.mesh.position.z - player.z) < CELL * 0.5) {
      playSound('powerup');
...
function startMusic() {
  ...
  setInterval(() => {
    arp.frequency.value = notes[noteIdx++ % notes.length];
  }, 150);
...
function startSplitScreen() {
  ...
  // Remove AI bombers for PvP
  aiBombers.forEach(b => scene.remove(b.mesh));
  aiBombers = [];
  
  // Clear all enemies for PvP
  for (const e of [...enemies]) {
    scene.remove(e.mesh);
  }
  enemies = [];
  ...
  // Create cameras
  cameras = players.map(() => {
    const cam = new THREE.PerspectiveCamera(80, (innerWidth/2) / innerHeight, 0.1, 100);
    return cam;
  });
  
  // Create player meshes
  const colors = [0x44ff66, 0xff6644];
  playerMeshes = players.map((p, i) => {
...
function updateSplitScreen(dt) {
  if (!locked) return;
  dt *= slowMo;
  
  // P1: WASD + Q/E turn + Space bomb
  const p1 = players[0];
  if (!p1.dead) {
    let dx = 0, dz = 0;
    if (keys['KeyW']) { dx -= Math.sin(p1.yaw); dz -= Math.cos(p1.yaw); }
    if (keys['KeyS']) { dx += Math.sin(p1.yaw); dz += Math.cos(p1.yaw); }
    if (keys['KeyA']) { dx -= Math.cos(p1.yaw); dz += Math.sin(p1.yaw); }
  ...
  // P2: Arrows + ,/. turn + Enter bomb
  const p2 = players[1];
  if (!p2.dead) {
    let dx = 0, dz = 0;
    if (keys['ArrowUp']) { dx -= Math.sin(p2.yaw); dz -= Math.cos(p2.yaw); }
    if (keys['ArrowDown']) { dx += Math.sin(p2.yaw); dz += Math.cos(p2.yaw); }
    if (keys['ArrowLeft']) { dx -= Math.cos(p2.yaw); dz += Math.sin(p2.yaw); }
  ...
  checkSplitPlayerHits();
}

function dropBombSplit(playerIdx) {
  ...
  const fuse = new THREE.Mesh(
    new THREE.CylinderGeometry(0.04, 0.04, 0.25),
    new THREE.MeshStandardMaterial({ color: colors[playerIdx], emissive: colors[playerIdx], emissiveIntensity: 1 })
  );
  fuse.position.y = 0.45;
  group.add(fuse);
  ...
  const sparkPositions = new Float32Array(30 * 3);
  sparkGeo.setAttribute('position', new THREE.BufferAttribute(sparkPositions, 3));
  const sparks = new THREE.Points(sparkGeo, new THREE.PointsMaterial({ color: colors[playerIdx], size: 0.08, transparent: true }));
  sparks.position.y = 0.55;
  group.add(sparks);
  
  const glow = new THREE.PointLight(colors[playerIdx], 1, 4);
  glow.position.y = 0.3;
  group.add(glow);
  
  group.position.set(gx, 0.4, gz);
  scene.add(group);
  bombs.push({ group, fuse, sparks, glow, time: 3, owner: p, range: p.blast, passable: true });
}

function checkSplitPlayerHits() {
  for (const exp of explosions) {
    if (exp.isParticle || exp.isDebris) continue;
    players.forEach((p, i) => {
      if (p.dead) return;
      if (Math.abs(exp.mesh.position.x - p.x) < CELL*0.6 && Math.abs(exp.mesh.position.z - p.z) < CELL*0.6) {
        p.dead = true;
...
function splitWin(winner) {
  locked = false;
  const title = winner === -1 ? 'DRAW' : `PLAYER ${winner + 1} WINS`;
  document.querySelector('#win-screen h1').textContent = title;
  document.querySelector('#win-screen h1').style.color = winner === 0 ? '#44ff66' : winner === 1 ? '#ff6644' : '#fff';
